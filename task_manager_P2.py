...
#only admin is allowed to register new users, view completed tasks and delete tasks
...
try:
    #read data from file and clean data
...
while True:
    ...
    #I took the liberty of modifying the standard menu to be a bit more aesthetic than in the template
    ...
    if username == "admin":
        ...
        print("5. Enter \"vc\" to view completed tasks")
        print("6. Enter \"del\" to delete tasks")
        print("7. Enter \"e\" to exit")

    #menu for everyone else
    else:
        print("\n**** Main Menu ****")
        print("1. Enter \"a\" to add a new task")
        print("2. Enter \"va\" to view all tasks")
        print("3. Enter \"vm\" to view my tasks")
        print("4. Enter \"e\" to exit")
    
    #convert user selection to lowercase to avoid errors
    menu = input("Enter selection: ").lower()

    #This option, previously available to all users should is now only available for admin
    if menu == 'r' and username == "admin":
        '''This code block will add a new user to the user.txt file
        - Check if the new p/w and old p/w are the same,then add them to the user.txt file or present error message'''
        ...
        '''This code block will allow a user to add a new task to task.txt file
              -Then, get the current date, and ad the data to the file task.txt
        '''
        ...
        with open(FILE_NAME2, "a", encoding="utf-8") as file:
            file.write(f"\n{task_user}, {task_title}, {task_desc}, {date_assigned}, {due_date}, {completed}")
        ...
        '''This code block will read the task from task.txt file and print to the console in the format of Output 2 presented in the PDF
        '''
    ...
    #if selects to view only their tasks
    elif menu == 'vm':
    ...
    #an option for admin to view only completed tasks
        ...
        with open(FILE_NAME2, "r", encoding="utf-8") as file:
            for line in file:
                ...
                #display task data in desired format if marked as complete
                if contents[5].strip().lower() == "yes":       
                    print("\n-------------------------------")
                    print(f"Assigned to:\t {contents[0]}")
                    print(f"Task:\t\t {contents[1]}")
                    print(f"Description:\t {contents[2]}")
                    ...
                    print("Unfortunately, no tasks have been completed!")
        

    #an option for admin to delete tasks 
        ...
        #incase no tasks exist in file
        if not tasks:
            print("Admin, there are no tasks available to delete.")
            continue

        print("\nSelect a task to delete:\n")

        #numbering the tasks for easy and accurtate selection
        for i, line in enumerate(tasks, start=1):
            contents = line.strip().split(", ")

        #summary view for identification
            print(f"""
                Task {i}
                Assigned to: {contents[0]}
                Task title: {contents[1]}
                Date assigned: {contents[3]}
                Due date: {contents[4]}
                Completed: {contents[5]}
                """)

        try:
            #prompt admin to select task to delete or to cancel
            index = int(input("Enter task index to delete (enter -1 to cancel): "))

            #cancel operation
            if index == -1:
                print("\nTask deletion cancelled succesfully.")
                continue
            ...
            if 1 <= index <= len(tasks):
                ...
                # Overwrite the text file without newly deleted task
                with open(FILE_NAME2, "w", encoding="utf-8") as file:
                    file.writelines(tasks)

                print("The selected task has been successfully deleted.")
                print("Removed task:", deleted_task)

            else:
                print("Admin, you entered an invalid task index number.")

        #incase of bad input
        except ValueError:
            print("Please enter a valid task index number.")


    #incase any user decides to exit
    elif menu == 'e':
        print('Goodbye!!!')
        exit()
